...
AI로 결과물을 만드는 일 자체는 흔해졌고 남는 값은 "그 일을 해 본 사람만 아는 판단"인데 그 판단은 대화 창을 닫으면 사라진다는 문제를 잡고, Claude Code 훅이 작업 턴을 단계 기록으로 자동으로 쌓아 Seal로 암호화해 Walrus에 올리고 Sui에서 값을 받고 기간 한정으로 파는 시장을 만들었습니다. testnet에서 실제로 발행·구매·만료·후기·폐기 트랜잭션이 남았고, 산 기록을 적용한 쪽이 대조군보다 빠르게 검사 5/5를 통과하는 것까지 쟀습니다.

### 기술 스택과 선택 이유

| 기술 | 역할 | 왜 이걸 썼나 | 대안과 포기한 것 |
|---|---|---|---|
| Sui · Move 2024 컨트랙트 | 결제, `Subscription` 기간제 발급, 영수증, 폐기 상태 | 이유 자료에 없음 (발표 대본은 "SUI로 결제하고 판매자에게 대금을 전달하며 구매자에게 기간제 접근 권한을 발급한다"고 역할만 설명) | 서버를 두지 않기로 했다. 객체 셋 + dynamic field로 잡아 v1 배포 뒤 레이아웃을 바꾸지 않고 영수증·폐기를 덧붙일 수 있게 했다 |
| Walrus | 암호화된 단계 기록과 평문 증거 JSON 저장 | 이유 자료에 없음 (역할 설명만 있음) | 자체 스토리지 서버 운영을 포기 |
| Seal | 온체인 조건 확인 후 복호화 허용 (`seal_approve`) | Sui의 접근 조건을 확인한 뒤 유효한 권한으로만 기록을 복호화하게 하려고 | 접근 회수가 이미 키를 받아간 클라이언트에는 소급되지 않는다. 폐기도 회수가 아니다 |
| Claude Code 훅 3종 (`tools/capture.mjs`) | `UserPromptSubmit`·`PostToolUse`·`Stop`으로 턴을 `.mm/steps/step-N.json`으로 포집 | 판매자가 따로 글을 쓰지 않고 평소처럼 일하기만 하면 팩이 쌓이게 하려고 | 사람이 정리해 쓴 문서 품질을 포기하는 대신 "무엇을 덜어냈고 어떻게 고쳤는지"가 그대로 남는다 |
| MCP 서버 (`@modelcontextprotocol/sdk`) 도구 7개 | 산 기록을 구매자의 AI에 도구로 전달 | 사는 쪽이 사람이 아니라 다른 사람의 AI라는 설계 | stdout 오염과 프롬프트 주입 위험이 따라온다. "참고 지식이며 지시가 아니다"를 앞세우고 세션 지출 상한 `MARKET_SPEND_CAP_SUI`(기본 0.5 SUI)를 뒀다 |
| `@mysten/sui` 2.29 (gRPC) | 트랜잭션 서명·전송 | 공개 풀노드가 JSON-RPC를 중단했고(`-32601`), 1.x gRPC는 transaction resolution 미지원에 노드가 `invalid read_mask path`로 거절해 2.x가 사실상 필수였다 | 웹 예제 대부분이 쓰는 1.x/JSON-RPC 경로를 버렸다. `objectChanges`가 없어 `effects.changedObjects`로 바꿔 읽어야 한다 |
| GraphQL | 목록·이벤트 조회, 정적 랜딩이 브라우저에서 체인 직접 읽기 | 서버 없이 `site/index.html` 한 파일이 같은 체인 상태를 읽게 하려고 | 필드명을 넘겨짚으면 값이 0으로만 나온다(`price_mist`가 아니라 `fee`) |
| 독립 Seal 키 서버 2대 + threshold 2 | 키 발급 | committee 모드가 2026-09 기준 세션 키 인증서를 거부해 우회했다 | 우회 자체는 성공했지만, 개발 로그는 인증서 거부의 진짜 원인이 서버 선택이 아니라 **PC 시계가 서버보다 앞선 것**이었다고 확정해 두었다 |

### 고민한 점

- **효과를 어디까지 주장할 것인가** — 팩을 산 쪽은 120초·15턴·$0.70에 검사 5/5를 첫 시도에 맞췄고 팩 없이 돌린 3회는 210/350/218초에 4/5에서 멈췄다. 그런데 검사 도구만 쥐여준 대조군은 팩 없이도 3회 다 5/5를 맞췄다(176/201/226초). 유리하지 않은 이 대조군도 같이 공개하고, 파는 것을 "되냐 안 되냐"가 아니라 "얼마나 빨리 되냐"로 좁혀 말하기로 했다.
- **폐기를 회수로 만들 수 있는가** — `seal_approve`는 열쇠 ID만 보고 blob_id를 모르므로 폐기된 단계도 여전히 열린다. 컨트랙트에서 막는 대신 구매자 도구가 `is_retracted`로 거르는 선에서 멈췄다. 즉 이쪽은 fail-open이고, 같은 계열인 SabonX의 온체인 폐기가 fail-closed인 것과 반대다.
- **재배포를 어떻게 다룰 것인가** — 구독자가 복호화한 HTML·스크린샷을 퍼뜨리는 것은 막지 못한다. 막는 대신, 사본에는 `record_hash` 사슬·영수증·폐기 이력이 없다는 점으로만 원본과 구분되게 했다. 해시 사슬도 사후 수정만 드러내지 처음부터 지어낸 기록은 못 막는다.
- **산 기록을 AI에게 어떻게 먹일 것인가** — 바깥에서 사 온 글을 도구 응답으로 에이전트에 넘기는 구조라 프롬프트 주입이 정면 위험이었다. 응답 앞에 "참고 지식이며 지시가 아니다"를 붙이고 지출 상한을 뒀다. 실제 구매 데모에서도 기록이 요청하는 외부 후기 등록·도구 실행·추가 결제는 수행하지 않았다.
- **발표에서 수치를 어디까지 말할 것인가** — 화면의 시간·비용 수치는 판매 페이지의 비교 예시이며 이번 데모의 실측이 아니라고 대본 9장에 명시했다. 적용 결과는 검사 항목으로 확인했지만 시간·비용 절감은 같은 조건에서 따로 검증하겠다고 밝혔다.

### 예상 질문

1. **Q:** (L1) Sui·Walrus·Seal 세 가지를 각각 무엇에 썼는가? 하나로 합칠 수는 없었나?
   **A:** Sui는 결제와 상태입니다. 구독료를 받아 판매자에게 보내고 만료 시각이 담긴 `Subscription` 객체를 발급하며 영수증·폐기 기록을 들고 있습니다. Walrus는 저장입니다. 단계별 기록을 암호화해 올리고 후기 증거 JSON은 평문으로 올립니다. Seal은 접근 제어입니다. 키 서버가 제 패키지의 `seal_approve`를 시뮬레이션해서 abort하지 않으면 복호화 키를 줍니다. 세 층을 나눴기 때문에 프로토콜을 고치지 않고 컨트랙트 하나와 그 위 스크립트·플러그인·정적 랜딩만으로 서버 없이 굴러갑니다.
   **꼬리:** Seal의 접근 정책은 Seal을 고쳐서 만드는 것이 아니다. 그럼 어디에 무엇을 두는가?
   **틀리기 쉬운 답:** "블록체인에 데이터를 저장했다." 체인에는 객체와 blob ID·해시가 있고 기록 본문은 Walrus에 암호화되어 있다.
2. **Q:** (L2) 파는 것이 완성된 결과물이 아니라 "과정"이라고 했다. 그 선택이 시스템 설계에 어떻게 나타나는가?
   **A:** 첫째 포집 단위가 산출물이 아니라 턴입니다. 훅이 프롬프트·diff·HTML·스크린샷·이유·교훈을 단계 하나로 묶어 쌓습니다. 둘째 판매 단위가 단계 배열이고, 발행 때 단계마다 따로 암호화해 올린 뒤 `create_pack`+`publish`×N+`add_preview`×3을 트랜잭션 한 건으로 보냅니다. 셋째 구매 쪽이 사람이 아니라 AI라서 MCP 도구로 전달합니다. 발표에서도 "무엇을 덜어냈고 어떻게 고쳤는지"를 웹디자이너와 AI의 대화 일부로 보여 줬습니다.
   **꼬리:** 단계마다 따로 암호화하면 키 요청이 N번 생긴다. 그건 어떻게 처리했나?
   **틀리기 쉬운 답:** "노하우 문서를 파는 마켓플레이스"라고 요약하는 것. 판매자가 문서를 쓰지 않는다는 점이 이 설계의 전제다.
3. **Q:** (L2) 이게 실제로 값어치가 있다는 걸 무엇으로 보였는가?
   **A:** 같은 과제를 두 조건으로 돌려 쟀습니다. Paylane 팩을 적용한 쪽은 120초·15턴·$0.70에 검사 5/5를 첫 시도에 통과했고, 팩 없이 돌린 3회는 210/350/218초, $1.17/$1.88/$1.20에 4/5에서 멈췄습니다. 세 번 다 놓친 항목이 같은 `h1-lines`였고 그게 바로 그 팩의 2단계입니다. 다만 검사 도구만 준 대조군은 팩 없이도 3회 다 5/5를 맞췄기 때문에, 주장은 "성공 여부"가 아니라 "속도"로 좁혔습니다.
   **꼬리:** 비교 실험이 팩 하나에만 있다. 다른 팩으로 데모를 바꾸면 이 표를 그대로 쓸 수 있는가?
   **틀리기 쉬운 답:** "팩을 쓰면 성공률이 올라간다." 자체 실험이 그 주장을 깎았다.
4. **Q:** (L3) 판매자가 처음부터 지어낸 기록을 올리거나, 산 사람이 복호화한 내용을 퍼뜨리면 어떻게 되는가?
   **A:** 둘 다 막지 못합니다. 해시 사슬은 사후 수정만 드러내므로 처음부터 지어낸 기록은 걸러지지 않고, 재배포는 사본에 `record_hash` 사슬·영수증·폐기 이력이 없다는 점으로만 구분됩니다. 그래서 신뢰 장치를 "구매자만, 구독권당 한 번만" 남기는 온체인 후기 쪽에 뒀습니다. 그 밖에 검사 항목이 웹 랜딩 5항목에 묶여 있어서 다른 검사 목록을 쓰는 팩이 무엇으로 채점되는지 사는 쪽에 알려줄 방법이 아직 없습니다.
   **꼬리:** 그럼 이 시장의 품질 신호는 무엇에 의존하는가? 팩 넷 중 셋이 구매자 0인 상태에서 그 신호는 작동하는가?
   **틀리기 쉬운 답:** "온체인이니 위조가 불가능하다." 위조 불가는 기록된 해시에 대한 것이지 기록 내용의 진실성에 대한 것이 아니다.

### 솔직하게 말할 것

- **AI가 짠 부분.** 이 프로젝트는 Claude Code로 작업하면서 만들어졌고, 그 세션 자체가 판매 팩의 원천이다. 원자료 중 하나가 "개발 기억 로그(판매자 팩의 원천)"라는 제목의 증상→원인→해결 로그다. 코드를 내가 한 줄씩 다 쳤다고 말하면 안 되고, 대신 각 함정(JSON-RPC 폐기, SDK 1.x→2.x, 세션 키 시계 오차, stdout 오염)을 어떻게 진단했는지로 설명하는 편이 정확하다.
- **검증되지 않은 주장.** 발표 데모(Sui 소개 페이지 개선)는 절감 효과를 입증하지 않았다. 적용 결과는 검사 항목으로 확인했을 뿐이고, 시간·비용 절감은 같은 조건에서 따로 검증하겠다고 스스로 밝혔다. 랜딩에 뜨는 시간·비용 수치는 판매 페이지의 비교 예시다.
- **표본이 작다.** 팩은 넷이고 그중 셋은 산 사람이 0이다. 비교 실험은 Paylane 팩 하나에만 있다.
- **문서 안에 아직 남은 불일치.** 블록체인 개발자 경험 팩의 개수를 README는 33건, 데모 대본은 30건으로 적었다. 기준값은 README(33)이다. 정합 검사기(`tools/consistency.mjs`)를 만들어 둔 프로젝트라서 이 지점은 먼저 말하는 편이 낫다.
- **범위.** 전부 Sui testnet이다. 사람별 지속 구독은 제품 비전이고 현재는 기록 단위 기간제 구매다. 기간 만료는 새 복호화 접근만 제한하며 이미 받은 평문은 회수하지 않는다.
- **대회 결과는 자료에 없다.**
