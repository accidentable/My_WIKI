...
사장님이 신메뉴를 찾으려고 SNS를 뒤지고 우리 가게에서 만들 수 있는지 따지고 재료를 사고 결과를 판단하는 과정에 시간·비용·판단 부담이 걸려 있다는 문제를 잡고, 이름이 아직 없는 유행 후보를 n-gram 버스트로 발견하고 검색지수 이동평균으로 단계를 판정한 뒤 그 가게의 재료·장비·예산에 맞춘 소규모 시험 판매안과 비용 계산까지 내놓는 AI 컨설팅을 만들었습니다. 결과물은 인기 순위가 아니라 "이런 메뉴가 나왔는데 우리 가게에서 이 방식으로 시험해보겠습니까"라는 가게별 실행 제안입니다.
...
| 순수 함수 `src/domain/cost.ts` | 시험 판매 비용(사전 현금·사용 원가·회수 수량) 계산 | 화면에서 돈에 해당하는 숫자는 처음부터 AI 밖에 두기로 했다 | 자연어 유연성을 포기하고 단위 테스트(T09~T13)로 고정 |
| `src/domain/store-fit.ts` (상한 검증자) | AI가 낸 적합도 점수·대체안을 규칙으로 다시 깎음 | 규칙이 점수를 계산하던 초기 구조에서 점수가 대부분 55%로 몰리고 이유도 고정 문장이었다. 2026-09-16에 판단을 AI로 옮기고 규칙을 상한 검증자로 남겼다 | 완전한 결정성을 포기. 대신 `to`가 실제 보유 재료일 때만 대체안을 인정하는 `verifySubstitutions`를 뒀다 |
| `openai` 7.15 / `@anthropic-ai/sdk` 0.125, 단일 `AI_API_KEY` | 메뉴 추출, 가게 적합도, 적용안 생성 | 범용 `OPENAI_API_KEY`/`ANTHROPIC_API_KEY`를 쓰면 다른 도구의 키를 실수로 쓸 수 있어서 키 변수를 하나로 한정했다 | 표준 변수명 관례를 포기. 호출은 최대 2회(SDK 재시도 0, 서비스 층 재시도 1) |
| 네이버 Search Trend·블로그 API, YouTube Data API, 구글 급상승 RSS | 유행 발견과 추적 | 공식 API와 공개 페이지만 쓴다. 인스타그램·배달앱·커뮤니티 크롤링은 약관 위반 소지로 제외했다 | 실제 SNS 관찰 게시물을 포기. 틱톡 Creative Center는 로그인 없이 40101을 반환해 보류, 구글은 공식 API가 없어 RSS를 교차 확인 신호로만 썼다 |
| 소진공 대구 상가정보 + 행안부 법정동 성·연령 인구 CSV | 반경 업소 집계와 동별 인구로 상권 맥락 제공 | 이유 자료에 없음 (DIP 카드·생활인구·주문 원본은 설명서만 확보했고 실제 데이터는 미확보) | 카드·주문 원본을 포기. 거주 인구를 방문객이나 특정 메뉴 수요로 해석하지 않는다는 원칙을 문서·화면에 박았다 |
| 블록체인 (미채택) | — | 대회명에 있다는 이유만으로 넣지 않고 초기 구현에서 제외했다 | 대회 키워드 가점을 포기. 금융 맥락은 시험 예산·구매 현금·사용 원가·비용 회수 계산으로 붙였다 |

### 고민한 점

- **화면에서 가장 큰 숫자를 누가 계산할 것인가** — 원래는 규칙이 적합도 %를 계산하고 AI는 추출과 레시피 초안만 썼다. 그 결과 점수가 대부분 55%로 몰리고 이유도 "재료 3개를 사야 해요" 같은 고정 문장이었다. 최종 커밋에서 역할을 뒤집어 AI가 점수·한 줄 이유·재료 대체안을 판단하고 규칙은 상한 검증자로만 쓰게 했다. 반대로 비용 계산은 끝까지 순수 함수에 남겼다.
- **실시간 요구와 운영자 검토 원칙의 충돌** — 순위의 이름을 눌렀는데 아직 후보가 아니면 `materializeKeyword`가 대표 글 3건을 자동 등록·추출·승인해 컨설팅으로 넘긴다. 상세기획서의 "운영자 검토 후 노출" 원칙에서 벗어나는 지점이고, 사용자 요구에 따른 결정이라고 기록해 두었다. 근거 인용 검사와 잘못된 항목 보류는 남겼지만 통제는 사후('순위에서 숨기기')만 남는다.
- **스파이크형 유행을 어떻게 처리할 것인가** — 버터떡은 12일 만에 정점을 찍어 감지일이 곧 정점일이었다. 재료를 사기 전에 끝나므로 예측 정확도를 올리는 대신 아예 후보에서 제외하는 필터를 넣었다.
- **되돌리기 검증이 근사치라는 것** — 네이버 블로그 API는 날짜 필터가 없고 최신 1,000건만 주므로 과거 시점의 버스트 순위가 샘플 편향에 흔들린다. 발견 층의 정량 평가는 실운영(일별 누적)에서만 가능하다고 범위를 좁혔다.
- **조사 문서 자체를 다시 검산할 것인가** — 구현 단계 검토에서 조사 문서 4.1 표의 "7일 규칙" 열이 실제로는 14일 규칙 값이고, 3개 유행은 주 단위 자료이며, 피자설기 정점이 두 기준으로 둘로 적혀 있음을 찾아냈다. 자기 자료를 근거로 쓰기 전에 다시 계산해 본 것이 이 발견의 계기였다.

### 예상 질문

1. **Q:** (L1) 이 서비스가 사장님에게 최종적으로 내놓는 것은 무엇인가? 유행 순위 서비스와 무엇이 다른가?
   **A:** 최종 산출물은 가게별 실행 제안입니다. 후보 메뉴 하나에 대해 재료·레시피·필요 기계·가격과 예상 매출·경쟁사, 그리고 우리 가게 재료로 바꿔 넣는 대체안과 시험 판매 비용 계산이 붙습니다. 핵심 질문을 "이 메뉴가 정확히 얼마나 유행하는가"가 아니라 "우리 가게에서 이 방식으로 시험해보겠습니까"로 잡았기 때문에, 전국 인기 순위·판매량 예측·성공 확률 보장은 명시적으로 범위 밖에 뒀습니다. 후보 목록에 인기 점수나 성공 확률을 두지 않은 것도 의도된 선택입니다.
   **꼬리:** 그럼 사장님이 "그래서 이게 몇 등이냐"고 물으면 무엇을 보여주는가?
   **틀리기 쉬운 답:** "SNS 데이터로 뜰 메뉴를 예측한다." 예측이 아니라 시험 판매 의사결정을 돕는 것이 이 서비스의 범위다.
2. **Q:** (L2) 대회 이름이 AI Blockchain Challenge인데 블록체인을 안 썼다. 왜인가?
   **A:** 대회명에 있다는 이유만으로 넣지 않기로 하고 초기 구현에서 제외했습니다. 풀려는 문제가 소상공인의 신메뉴 시험 판매 의사결정이라 분산 원장이 해결하는 문제가 아니었고, 대신 공모 분야가 요구하는 금융 맥락은 시험 예산·구매 현금·사용 원가·비용 회수 계산으로 붙였습니다. 같은 대회의 다른 트랙에는 DID·VC와 온체인 폐기 레지스트리를 실제로 쓴 결과물(SabonX)을 따로 냈기 때문에, 블록체인을 못 써서 뺀 것이 아니라 여기서는 필요하지 않다고 판단한 것입니다.
   **꼬리:** 그래도 이 서비스에 블록체인을 넣는다면 어디에 넣겠는가? 그 이득은 무엇인가?
   **틀리기 쉬운 답:** "데이터 신뢰성을 위해 공공데이터를 온체인에 올린다." 원본이 공공기관 배포 파일이고 해시 대조 절차를 이미 파일로 갖췄으므로 체인이 더해주는 것이 거의 없다.
3. **Q:** (L2) 데모용 화면과 실제 AI가 만든 결과를 사용자가 어떻게 구분하는가?
   **A:** 데이터 층에서 갈랐습니다. `dataMode`, `acquisitionMethod=demo`, `generationMode`를 DB와 API에 두고 화면에는 "체험 모드 / 실사용 모드" 배지 하나로 노출합니다. 원칙은 키가 없을 때 고정 샘플을 실제 AI 생성처럼 보여주지 않는다는 것이고, 데모 어댑터의 추출과 적용안은 규칙 기반 초안이라고 문서에 적어 뒀습니다. 실제 경로도 키로 확인해서 OpenAI `gpt-5.6-terra`로 연결 테스트·추출·적용안 생성까지 돌렸습니다.
   **꼬리:** 데모 어댑터와 실제 AI의 출력 형식이 같아야 하는가, 달라도 되는가? 같게 만들면 무엇이 위험한가?
   **틀리기 쉬운 답:** "화면에 '데모'라고 써 두면 된다." 구분을 UI 문구가 아니라 DB·API 필드로 들고 있어야 나중에 감사할 수 있다.
4. **Q:** (L3) 내일 대구 카페 사장님 한 분과 파일럿을 시작한다면 무엇이 막는가?
   **A:** 로그인과 가게별 접근 제어(T18)가 P1으로 미구현이라 실제 사장님 데이터를 받는 파일럿은 아직 불가합니다. 자료 수집도 P0에서는 운영자 수동 등록 기반이고 자동 수집은 P1입니다. 데이터 쪽으로는 실제 SNS 관찰 게시물, 사장님 실입력 원가, 실제 시험 판매 결과가 전부 미확보라 비용 계산의 입력이 가정값입니다. Anthropic 어댑터는 구현돼 있지만 Anthropic 키로 검증하지 않았습니다.
   **꼬리:** 관찰 범위는 전국인데 초기 이용자는 대구 카페다. 인구·상가 데이터는 대구만 적재했다. 이 불일치는 결과에 어떻게 나타나는가?
   **틀리기 쉬운 답:** "테스트가 다 통과했으니 바로 쓸 수 있다." 단위 테스트 64개와 수용 테스트 통과는 구현 완결성의 증거이지 운영 준비의 증거가 아니다.

### 솔직하게 말할 것

- **F01~F09 "전부 완료"는 구현 보고서 자신의 표시다.** 외부 검증이 아니라 자기 점검 결과이고, 같은 표에 데모 어댑터가 규칙 기반이라는 단서가 붙어 있다.
- **검증 안 된 부분.** Anthropic 어댑터는 키로 돌려 보지 않았다. 발견 층(n-gram 버스트)의 정량 성능은 과거 데이터로 평가할 수 없어 근사치로만 확인했다.
- **기획 원칙을 스스로 깬 부분.** `materializeKeyword`의 자동 등록·승인은 "운영자 검토 후 노출" 원칙에서 벗어난다. 문서에 적어 뒀으니 면접에서도 먼저 말하는 편이 낫다.
- **마지막 날 방향을 뒤집었다.** 적합도 점수의 AI 이관은 최종 커밋일(2026-09-16) 작업이라, 바꾼 뒤의 출력 품질을 충분히 관찰한 기간이 없다.
- **자료에 없는 것.** 원자료에 AI 코딩 도구를 어디까지 썼는지에 대한 서술이 없다. 코드 작성 비중을 물으면 기억나는 대로 말하되 자료로 뒷받침되지 않는다고 밝히는 편이 안전하다.
- **접수 상태.** 원자료 기준으로 대회 접수(붙임1~4)와 제출 ZIP 업로드는 아직 하지 않은 상태였고, 수상 여부는 자료에 없다.
