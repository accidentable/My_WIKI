...
급여 처리를 위해 신분증 사본 전체를 사장님에게 넘기면 업무에 필요 없는 사진·주소까지 같이 간다는 문제를 잡고, 발급기관이 서명한 SD-JWT VC에서 이름만 선택 공개하고 주민등록번호는 모의 국세청만 열 수 있게 JWE로 봉인해 제출하는 급여 확인·소득 신고 흐름을 만들었습니다. 정보 변경으로 폐기된 VC는 서명이 유효해도 Sepolia 폐기 레지스트리 조회로 제출 단계에서 거절되는 것까지 데모로 보여 줍니다.
...
| SD-JWT VC (`@sd-jwt/sd-jwt-vc`) | 기관 서명 + 홀더 공개키 결합, 항목별 선택 공개 | 신분증 이미지 대신 기관이 서명한 자격증명을 쓰고 필요한 항목만 제출하려고 | 신분증 사본 전달 방식을 버렸다. 다만 SD-JWT는 공개하기로 한 값을 암호화하지는 않아 별도 보호가 더 필요했다 |
| JWE · X25519 (`jose`) | 주민번호를 `ECDH-ES+A256KW`/`A256GCM`으로 봉인 | 세무 처리 대상으로 전달할 주민번호에는 선택적 공개와 별개의 기밀성이 필요해서. 일용근로소득 지급명세서에는 세액이 0원이어도 주민번호 13자리가 들어간다 | 주민번호를 아예 빼는 안은 명세서 요건 때문에 포기 |
| `cnf.jwk` · KB-JWT | 제시자가 VC에 묶인 키의 소유자임을 nonce·aud·sd_hash로 증명 | 발급기관 서명만으로는 지금 제출한 사람이 정당한 소유자인지 알 수 없어서 | VC 복사·다른 키 서명은 막지만 개인키 자체가 탈취되는 문제는 못 막는다 |
| JWS 승인 JWT | `request_id`·`request_hash`·`presentation_hash`·`decision`을 묶어 서명 | 유효한 신원 제시가 특정 월·금액에 동의했다는 뜻은 아니고, 요청·제시 묶음 바꿔치기도 막아야 해서 | 제시 서명 한 번으로 끝내는 단순함을 포기. 금액이 달라진 다음 달 요청에는 새 승인이 필요하다 |
| Solidity · Sepolia · viem | 폐기 기록 쓰기, 제출 시 `revoked(statusIndex)` 읽기 | 주소가 바뀌어도 이전 VC의 서명은 수학적으로 유효하다. 화면 버튼만 막아서는 서버 차원의 폐기 검증이 되지 않아서 | 메인넷 대신 테스트넷. 조회는 `eth_call`이라 가스가 들지 않지만, RPC 제공자는 조회하는 식별자를 볼 수 있다 |
| IndexedDB · `idb-keyval` | 브라우저 지갑에 개인키와 VC 보관 | 서버에 발급 결과를 캐시하면 지갑 밖에도 전체 VC 사본이 남아서, 캐시를 없애고 메타데이터만 서버에 남겼다 | 외부 지갑 연동을 포기. 사이트 코드가 개인키를 읽을 수 있어 XSS·악성 배포에 대한 하드웨어 수준 보호가 없다 |
| Express · 메모리 Map | 역할 권한, 급여 요청, 정정 메시지, 접수 결과, 발급 메타데이터 | 이유 자료에 없음 | 영속 DB를 포기했다. 재시작하면 새 데모를 시작해야 하고 다중 인스턴스에는 DB 트랜잭션이 필요하다 |

### 고민한 점

- **선택적 공개만으로 충분한가** — SD-JWT는 공개하지 않을 항목을 숨기지만 공개한 값을 암호화하지는 않는다. 주민번호는 검증자에게 평문으로 가면 안 되고, 그렇다고 빼면 지급명세서를 만들 수 없다. 그래서 선택적 공개(공개 범위)와 JWE(전달 값의 기밀성)를 역할을 갈라 함께 쓰고, 암호문을 발급기관 서명이 보호하는 `rrn_sealed`에 넣었다. 사장님 개인키로도 국세청 공개키로도 열리지 않음을 보여주는 **항상 실패하는** 엔드포인트(`POST /api/verifier/:sessionId/open`)를 일부러 남겼다.
- **서버 VC 캐시를 없앨 것인가** — 캐시를 두면 지갑 밖에 전체 사본이 남고, 없애면 발급 응답을 잃었을 때 기존 VC를 다시 내려줄 수 없다. 캐시를 없애고 `Cache-Control: no-store` + 재발급(기존 증명 폐기 후 새 발급)으로 갔다. 결과적으로 서버 사본 동기화가 사라졌지만, 키까지 잃으면 현재 데모에서는 새 세션을 시작해야 한다.
- **폐기 조회가 실패하면 접수할 것인가** — RPC 장애로 폐기 상태를 못 읽었을 때 통과시키면 폐기 검증이 의미를 잃는다. 조회 실패에도 접수하지 않는 fail-closed를 택하되, 폐기와 네트워크 오류를 화면에서 구분하고 로컬 스텁일 때는 온체인 확인으로 표시하지 않기로 했다.
- **실기기 시계 차이** — 실기기 시연에서 브라우저의 `iat`가 서버 시각보다 앞서 발급이 거절됐다. JWT 검증에 60초 Clock Tolerance를 넣되, 30초 미래 요청은 허용하고 과도한 미래·만료 요청은 거절하는 테스트를 같이 둬서 시간 검증 자체를 없앤 것이 아님을 확인했다.
- **데모 편의와 검증 무결성** — 폐기 거절을 보여주려면 사용자가 일부러 이전 VC를 제출할 수 있어야 했다. '무시하고 제출하기'는 화면의 사전 제한만 건너뛰고 서버의 서명·폐기 검증은 그대로 받게 했다.

### 예상 질문

1. **Q:** (L1) 이 서비스에서 "신분증 사본을 넘기지 않는다"는 것이 구체적으로 무슨 뜻인가? 어떤 정보가 어디에 남는가?
   **A:** 신분증 이미지 대신 발급기관이 서명한 VC를 쓰고, 제출 시 이름 Disclosure만 공개하고 주민번호는 JWE로 봉인해 보냅니다. 브라우저 지갑에는 개인키와 VC가, 서버 발급 메타데이터에는 공개키·폐기 식별자·만료 시각·변경 상태만 남고 전체 VC 사본은 없습니다. Sepolia에는 폐기 식별자와 상태·이벤트만 올리고 이름·주소·주민번호 원문은 올리지 않습니다. 다만 데모 발급용 모의 기관 원천 데이터는 서버에 남아 있어서, "데모 서버 전체에 주민번호가 없다"고는 말하지 않습니다.
   **꼬리:** 제출된 VP와 복호화한 주민번호는 왜 업무 기록에 저장하지 않는가? 처리 중 일시 보유와 지속 보관을 어떻게 나눴나?
   **틀리기 쉬운 답:** "블록체인에 올리니까 안전하다." 체인에 올라간 것은 폐기 식별자뿐이고, 개인정보는 오히려 체인에 올리지 않는 것이 설계의 핵심이다.
2. **Q:** (L2) 폐기 목록을 그냥 서버 DB에 두면 되지 않나? 왜 굳이 체인에 올렸는가?
   **A:** 폐기 기록을 여러 검증자가 같은 기준으로 확인할 수 있게 하려고 체인을 썼습니다. 발급기관 권한으로 `revoke(statusIndex)`를 보내고, 검증기는 제출 때마다 `revoked(statusIndex)`를 `eth_call`로 읽습니다. 다만 솔직히 말하면 이번 데모에서는 요청 관리·발급기관·검증기가 하나의 Node.js 프로세스에서 돌기 때문에 "여러 검증자"라는 이점이 실제로 실현된 구성은 아닙니다. 블록체인이 개인정보의 진실성이나 기관의 정직함을 자동으로 보장하지도 않습니다.
   **꼬리:** 기관이 악의적으로 멀쩡한 VC를 폐기해 버리면 이 구조는 무엇을 보장하는가?
   **틀리기 쉬운 답:** "탈중앙화라서 위변조가 불가능하다"로 끝내는 답. 여기서 체인이 보장하는 것은 폐기 기록의 공통 조회 기준이지, 기록 내용의 진실성이 아니다.
3. **Q:** (L2) 데모에서 발급기관·요청 관리·검증기가 한 프로세스에 있다. 그런데도 이 프로토타입이 무엇을 증명했다고 말할 수 있는가?
   **A:** 증명한 것은 신뢰 경계의 분리가 아니라 **암호 절차의 결합이 실제로 동작한다**는 것입니다. 발급·선택 공개·JWE 봉인·키 바인딩·승인 결합·폐기 조회·중복 제출 차단은 실제로 구현했고, 통합 테스트(`npm run test:payroll`)가 잘못된 키·승인 변조·중복 제출 거절, 이전 VC 거절과 재발급 접수, 시계 오차 허용 범위까지 확인합니다. 기관 키가 인프라 수준으로 격리되지 않았다는 점은 README에 한계로 적어 두었습니다.
   **꼬리:** 그럼 실제로 기관을 분리하려면 무엇부터 바꿔야 하는가? 키 관리와 API 경계 중 어디가 먼저인가?
   **틀리기 쉬운 답:** 아키텍처 그림의 역할 분리를 곧 실제 격리로 설명하는 것. 그림은 논리적 역할 구분이다.
4. **Q:** (L3) 이걸 실서비스로 올린다면 어디가 가장 먼저 깨지는가?
   **A:** 첫째 상태 저장입니다. 서버가 메모리 Map이라 재시작하면 데모를 새로 시작해야 하고, 요청은 데모당 50건, 세션은 24시간·서버당 최대 200개로 묶여 있습니다. 다중 인스턴스에서는 동시 제출 방지를 세션 잠금이 아니라 DB 트랜잭션으로 옮겨야 합니다. 둘째 지갑입니다. 브라우저 내장 지갑이라 사이트 코드가 개인키를 읽을 수 있고, 기기 변경·키 복구 경로가 없습니다. 셋째 알림이 폴링이고 SMS·푸시·송금·세액 계산은 전부 모의입니다.
   **꼬리:** 키 복구를 넣으면 "지갑 밖에 전체 VC 사본을 두지 않는다"는 원칙과 충돌한다. 어떻게 타협하겠는가?
   **틀리기 쉬운 답:** "DB만 붙이면 된다." 동시성·복구 설계와 키 복구가 같이 와야 하고, 후자는 개인정보 보관 원칙과 정면으로 부딪친다.

### 솔직하게 말할 것

- **AI가 짠 부분.** 초기 구현은 Claude Code, 현재 앱 UI·급여 승인·변경 요청·폐기 결과 화면·테스트·문서 개선은 OpenAI Codex가 작성했다. 문제 정의와 시연 방향은 사람이 정하고 결과를 검토·수정했다. 암호 라이브러리(`jose`, `@noble/curves`, `@sd-jwt/sd-jwt-vc`)를 조합한 것이지 새 알고리즘을 만든 것이 아니다.
- **검증되지 않은 것.** 로컬 스텁 테스트 통과가 실제 Sepolia 쓰기 성공을 뜻하지 않는다고 README가 스스로 밝혀 두었다. 온체인 경로는 환경변수가 없으면 LOCAL STUB으로 돈다.
- **모의인 부분.** 기관의 신원 확인, 이름·주소 변경 승인, 전화번호 라우팅, 세무 접수는 전부 모의다. 실제 SMS·푸시·송금·세액 계산·납부는 없고, 공식 기관 연동과 법적 효력은 범위 밖이다.
- **`/legacy/*` 단계별 검증 API 흐름**은 현재 앱(`server/payrollRoutes.ts`) 이전의 검증 데모 경로로 **보인다(추정)**. 이 흐름에서는 검증자가 국세청이 아니라 사장님이라 지금 앱과 역할 배치가 다르다. 면접에서 두 흐름을 섞어 말하지 않도록 주의.
- **대회 결과는 자료에 없다.** 같은 구현을 2026 AI Blockchain Challenge in Daegu와 TRUST404 Track 02 두 곳에 냈다는 사실은 밝히는 편이 낫다. 두 README의 차이는 배지와 로컬 실행 안내뿐이다.
