...
### 설명할 수 있어야 하는 것
- 서명이 수학적으로 유효한 옛 VC를 무효로 만들려면 서명 바깥에 상태 저장소가 필요했고 여러 검증자가 같은 기준으로 봐야 해서 체인을 썼다고 설명할 수 있습니다.
- 화면 버튼 차단은 서버 검증이 아니라서 버렸고, 인덱스만 올려 개인정보 노출을 피하는 대신 RPC 제공자에게 조회 식별자가 보이는 것은 감수했다고 말할 수 있습니다.
- RPC 장애가 나면 fail-closed라 정상 사용자도 접수되지 않는다는 점과, 가용성을 정합성과 맞바꾼 이유를 설명할 수 있습니다.

### 확인 질문
1. **Q:** (L1 개념) VC 폐기를 체인에 따로 기록해야 하는 이유가 무엇이고, 무엇을 올리고 무엇을 올리지 않으셨나요?
   **A:** VC의 서명은 발급 시점의 내용에 대한 것이라, 주소가 바뀌어도 옛 VC의 서명은 수학적으로 여전히 유효합니다. 그래서 이 자격증명이 더는 유효하지 않다는 사실은 서명 바깥의 상태 저장소에 있어야 한다고 봤습니다. 이 프로젝트에서는 VC에 발급기관 서명으로 들어간 폐기 식별자 `statusIndex`만 체인에 올리고, 상태와 이벤트 말고 이름이나 주소, 주민등록번호 원문은 올리지 않았습니다. 체인을 쓴 건 여러 검증자가 같은 폐기 기록을 같은 기준으로 확인하게 하려는 이유였습니다.
   **꼬리:** 그러면 인덱스만 올려도 남는 프라이버시 문제는 무엇일까요?
   **틀리기 쉬운 답:** "블록체인에 올렸으니 개인정보의 진실성도 보장된다"라고 답하는 경우가 있는데, 실제로 체인은 기록의 공유와 변조 저항을 줄 뿐 기관이 정직한지까지 보장하지 않습니다.
2. **Q:** (L2 판단) 폐기 상태 조회에 실패했을 때 접수를 막는 fail-closed를 고르신 이유가 있나요?
   **A:** 조회 실패를 폐기 아님으로 처리하면 공격자가 RPC를 방해하는 것만으로 폐기된 VC를 통과시킬 수 있습니다. 그래서 검증기가 `revoked(statusIndex)`를 읽어 폐기됐거나 조회에 실패하면 접수하지 않게 했습니다. 다만 화면에서는 둘을 갈라서, 폐기가 확인된 경우에만 거절 화면을 띄우고 RPC 장애를 폐기로 표시하지는 않았습니다. 체인 환경변수가 없으면 LOCAL STUB 목록을 쓰되 그때는 온체인 확인으로 표시하지 않도록 `GET /api/chain`의 `isStub`을 뒀습니다.
   **꼬리:** 그러면 fail-closed 때문에 정상 사용자가 막히는 상황은 어떻게 덜어 주시겠어요?
   **틀리기 쉬운 답:** "조회 실패는 일시적이니 통과시키고 나중에 재검증하면 된다"라고 답하는 경우가 있는데, 실제로 접수 자체가 결과를 만드는 흐름에서는 되돌릴 수 없습니다.
3. **Q:** (L3 한계) 비슷한 방식인데 Memory Market의 `retract`는 왜 fail-open인가요? 무엇이 둘을 가르나요?
   **A:** 판매자가 낡은 단계를 `retract`하면 `RetractKey{blob_id}` → `Retraction{reason, at_ms}`가 팩에 남는데, 접근 승인 함수 `seal_approve`는 열쇠 ID만 보고 blob_id를 몰라서 폐기를 검사하지 않습니다. 그래서 폐기된 단계도 구독자에게는 여전히 열리고, Move 테스트에 그대로 적혀 있으며, 구매자 도구가 `is_retracted`로 걸러 줍니다. 폐기 판정이 검증기 안에 있으면 fail-closed가 되고 클라이언트에 있으면 fail-open이 된다고 정리했습니다. Memory Market의 폐기는 회수가 아니라 더는 권하지 않는다는 표식입니다.
   **꼬리:** 그러면 `seal_approve`가 blob_id를 볼 수 있게 하려면 열쇠 ID 설계를 어떻게 바꾸시겠어요?
   **틀리기 쉬운 답:** "온체인에 폐기 기록이 남으니 접근도 막힌다"라고 답하는 경우가 있는데, 실제로 기록과 집행은 별개입니다.
