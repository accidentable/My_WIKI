...
### 설명할 수 있어야 하는 것
- 위경도 차이를 `dLng * cos(lat) * R` / `dLat * R`로 미터 오프셋으로 바꾸고, AR에서 -z가 북쪽이라 남북 성분의 부호를 뒤집으며 y를 1.5로 고정한 각 선택의 근거
- 오브젝트 배치에는 접평면 소각 근사를, 반경 100m 진입 판정과 방위각에는 Haversine·`atan2`를 따로 쓰는 이유와, 원자료가 계획 문서라 이 값들이 아직 실행에서 검증되지 않았다는 점
- ViroReact가 Expo Go에서 안 돌아 `expo prebuild`가 필요하고 iOS AR은 시뮬레이터에서 안 되며 GPS는 실외에서만 정확하다는 제약이, "빨리 돌려보는" 개발 경로를 어떻게 막는지
