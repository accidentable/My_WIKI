...
### 설명할 수 있어야 하는 것
- 왜 라이브러리 대신 공개 엔드포인트를 직접 호출했는가 (서버리스 번들 경량화, 응답 형태 통제)
- 대안은 무엇이었고 무엇을 포기했는가 (유료 API·yfinance / 안정성을 포기하고 무과금과 가벼움을 얻음)
- 어떤 조건에서 이 방식이 깨지는가 (페이지 리뉴얼, 엔드포인트 형식 변경, 세 소스 동시 실패)

### 확인 질문
1. **Q:** (L1 개념) 실패를 흡수하는 세 겹이 각각 어떤 층위의 실패를 막는가?
   **A:** 첫째는 티커 단위 격리로, `_fetch_one`이 개별 예외를 잡아 `(key, None)`을 돌려주고 `gather(return_exceptions=True)`로 한 종목 실패가 나머지를 죽이지 않게 한다. 둘째는 소스 단위 fallback으로, 결과가 비면 하드코딩 상수(`YAHOO_FALLBACK`, 휘발유 3.45, 지지율 41.3)로 대체한다. 셋째는 엣지 캐시로, 서버리스 응답에 `Cache-Control: s-maxage=30, stale-while-revalidate=60`(risk-level), 120/300(truths), 300/600(history)을 붙여 Yahoo로 나가는 실호출을 캐시 주기당 1회로 줄이고 재검증 중에는 낡은 값이라도 즉시 응답한다.
   **꼬리:** `s-maxage`와 `max-age`의 차이는? 왜 서버리스 API 응답에 `s-maxage`를 쓰는가?
   **틀리기 쉬운 답:** "캐시는 속도를 위한 것" — 여기서는 업스트림 호출량을 눌러 무료 엔드포인트가 막히지 않게 하는 장치이기도 하다.

2. **Q:** (L2 판단) `yfinance`를 쓰지 않고 `query1.finance.yahoo.com/v8/finance/chart/{ticker}`를 httpx로 직접 친 이유는?
   **A:** 서버리스 번들이 가벼워지고(pandas 미포함) 응답 형태를 직접 통제할 수 있기 때문이다. 브라우저 User-Agent를 붙이고 `follow_redirects=True`로 호출해 `chart.result[0].meta`에서 `regularMarketPrice`, `chartPreviousClose`(없으면 `previousClose`), `fiftyTwoWeekHigh`만 꺼내 쓰고, 티커 5~7개를 `asyncio.gather`로 동시에 친다. 대가는 비공식 경로 의존이다 — Yahoo 공개 엔드포인트는 계약이 아니라 언제든 형식이 바뀌거나 막힐 수 있어 데모·개인 프로젝트 수준에서 쓰는 게 안전하다.
   **꼬리:** 응답 스키마가 바뀌면 이 코드는 어떤 증상으로 나타나는가?
   **틀리기 쉬운 답:** "라이브러리를 쓰면 더 안전하다" — 같은 비공식 엔드포인트를 감싼 라이브러리라면 깨지는 시점은 같고, 깨졌을 때 고치기가 오히려 어렵다.

3. **Q:** (L2 판단) 지수를 절대값이 아니라 52주 고점 대비 하락률로 바꿔 쓴 이유는?
   **A:** 지수 레벨은 시대에 따라 의미가 달라지지만 고점 대비 낙폭은 비교 가능한 값이 되기 때문이다. 계산은 `(현재가 - 52주고점) / 52주고점 * 100`이고, 이렇게 만든 하락률이 [[market-redline-composite-score]]의 safe=-3 / redline=-15 구간에 그대로 들어간다. 즉 이 변환은 수집 단계의 편의가 아니라 점수화 가능한 단위를 만들기 위한 선택이다.
   **꼬리:** 52주 고점이 최근에 갱신됐다면 이 지표는 어떻게 움직이는가?
   **틀리기 쉬운 답:** "지수 포인트를 그대로 쓰면 된다" — 2000포인트와 6000포인트 시대의 같은 숫자는 같은 위험이 아니다.

4. **Q:** (L3 한계) fallback이 있는데도 이 설계가 위험한 이유는 무엇이고, 무엇을 고쳐야 하는가?
   **A:** fallback이 장애를 숨기기 때문이다. 세 소스가 모두 실패해도 화면은 그럴듯한 숫자를 보여주는데, TACO Index에는 "이 값은 fallback입니다" 플래그가 응답에 없어서 사용자도 개발자도 지금 보는 지지율 41.3이 실측인지 상수인지 구분할 수 없다. 최소한 응답에 `is_fallback` 같은 필드를 두거나 `updated_at`을 실제 fetch 성공 시각으로만 갱신해야 한다. 특히 스크래핑은 조용히 깨진다 — 문자열 위치 + 정규식 조합은 페이지 리뉴얼 한 번에 무력화되는데, 예외가 아니라 "못 찾음 → fallback"으로 빠지므로 에러 로그도 거의 안 남는다.
   **꼬리:** `updated_at`만 고쳐도 탐지가 되는가? 모니터링을 하나 더 붙인다면 무엇을 재겠는가?
   **틀리기 쉬운 답:** "fallback이 있으니 가용성이 높다" — 값이 항상 돌아오는 것과 값이 맞는 것은 다르다.

5. **Q:** (L3 한계) 여러 소스를 하나의 히스토리 차트로 합칠 때 생긴 문제는?
   **A:** 두 가지다. 같은 대상에 다른 티커를 쓰면 축이 어긋난다 — 실시간은 `ES=F`(E-mini 선물), 히스토리는 `^GSPC`(현물)를 써서 같은 차트에 성격이 다른 두 계열이 섞였고, 선물은 야간에도 움직이는데 현물은 아니다. 또 거래일이 티커마다 달라 유가 선물과 지수의 거래일이 어긋나면서 날짜 축에 구멍이 생겼다. TACO Index는 날짜를 키로 합친 뒤 직전 값으로 forward-fill하고, 그래도 전 지표가 안 채워진 날은 버렸다.
   **꼬리:** forward-fill로 채운 날의 총점은 어떤 의미로 읽어야 하는가?
   **틀리기 쉬운 답:** "결측은 보간하면 해결" — 보간된 값으로 계산된 총점은 관측이 아니라 가정이고, 차트에는 그 구분이 남지 않는다.
