...
2026 AI Blockchain Challenge in Daegu 제출용 프로토타입입니다. 소상공인과 근로자 사이의 신분증 사본 전달을 줄이는 블록체인 기반 신원 검증 서비스를 제안합니다. 카페에서 정기적으로 일하는 근로자의 월 급여 확인을 중심으로, 선택적 공개·암호화된 정보 전달·온체인 폐기를 시연합니다. 실제 정부기관 연동이나 법적 신고 처리는 하지 않습니다.
...
기본 근로자는 가상 인물이고, 사업장은 **카페 온유**, 테스트 전화번호는 `010-0000-1234`입니다. 모든 신원정보는 가상 데이터입니다.
...
ZIP을 압축 해제한 뒤 `SabonX-iM-Challenge` 폴더에서 실행하세요. Node.js 22.12 이상과 npm이 필요합니다. 최초 의존성 설치에는 인터넷 연결이 필요합니다.

기본 시연에는 `.env`나 개인키가 필요하지 않습니다. 온체인 시연을 원할 때만 본인의 기관 키와 폐기 레지스트리를 설정하세요. 기존 운영 서버의 비밀키는 포함하지 않았습니다.

제출본은 별도의 `sabonx` 저장 키와 JWT 식별자를 사용합니다. 기존 배포본의 세션·VC는 이 버전으로 가져올 수 없으므로 새 데모를 시작하세요.
...
실행과 시연은 이 README, 내부 흐름은 [앱 구현 문서](docs/APP-DEMO.md), 서버·컨트랙트 배포는 [배포 안내](docs/DEPLOY.md)를 참고하세요.
